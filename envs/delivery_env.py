...
class DeliveryRobotEnv(gym.Env):
    ...
    def __init__(self, render_mode=None, width=800, height=600, robot_speed=5.0):
        ...
        #     (6, 2),  # Printer
        # ]
        # self.obstacles = [(grid_to_pixel(x, y)[0], grid_to_pixel(x, y)[1], 15) for x, y in obstacle_positions]
    ...
    def _get_obs(self):
        ...
        return np.array([self.robot_pos[0], self.robot_pos[1], self.robot_angle] + delivery_status, dtype=np.float32)
    ...
    def render(self):
        ...
        for block_x, block_y, block_w, block_h in self.blocked_areas:
            # Fill with gray
            pygame.draw.rect(self.window, (180, 180, 180), (block_x, block_y, block_w, block_h))
            ...
            for i in range(0, block_w + block_h, 8):
                ...
                if start_x <= block_x + block_w and end_y <= block_y + block_h:
                    pygame.draw.line(self.window, (120, 120, 120), (start_x, start_y), (end_x, end_y), 2)

        # Draw carpets with 'C' label
        for carpet_x, carpet_y, carpet_w, carpet_h in self.carpets:
            pygame.draw.rect(self.window, (139, 69, 19), (carpet_x, carpet_y, carpet_w, carpet_h))
            # Add carpet texture
            for i in range(0, carpet_w, 15):
                for j in range(0, carpet_h, 15):
                    pygame.draw.circle(self.window, (160, 82, 45), (carpet_x + i + 7, carpet_y + j + 7), 3)
        ...
        for wall_x, wall_y, wall_w, wall_h in self.walls:
            pygame.draw.rect(self.window, (0, 0, 0), (wall_x, wall_y, wall_w, wall_h))

        # Draw delivery tables
        for i, (table_x, table_y, table_w, table_h) in enumerate(self.tables):
            color = (0, 200, 0) if i in self.delivered_tables else (255, 165, 0)  # Green if delivered
            pygame.draw.rect(self.window, color, (table_x, table_y, table_w, table_h))
            pygame.draw.rect(
                self.window,
                (0, 0, 0),  # Black border
        ...
        for obs_x, obs_y, obs_radius in self.obstacles:
            pygame.draw.circle(self.window, (255, 100, 100), (int(obs_x), int(obs_y)), obs_radius)
            pygame.draw.circle(
                self.window,
                (200, 0, 0),  # Darker border
                (int(obs_x), int(obs_y)),
                obs_radius,
                2,
            )
        ...
        # Draw robot
        pygame.draw.circle(
            self.window, (0, 100, 255), (int(self.robot_pos[0]), int(self.robot_pos[1])), self.robot_radius
        )
        ...
        pygame.draw.line(
            self.window, (0, 50, 150), (int(self.robot_pos[0]), int(self.robot_pos[1])), (int(end_x), int(end_y)), 3
        )
