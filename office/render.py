...
RENDER_CONFIG = {
    ...
    'image_paths': {
        'background': "office/bing.png",
        'walls': "",
        'tables': {
            'delivered': "office/wood_g.jpg",
            'pending': "office/wood_r.jpg"
        },
        'obstacles': "",
        'carpets': "office/carpet.jpg",
        'robot': "",
    },
...
def render_environment(env):
    ...
    if RENDER_CONFIG['use_images']['tables']:
        ...
        for i, table in enumerate(env.tables):
            ...
            if img:
                ...
                # Fallback to color
                color = RENDER_CONFIG['colors']['tables']['delivered'] if i in env.delivered_tables else RENDER_CONFIG['colors']['tables']['pending']
    ...
    if env.show_obstacles:
        if RENDER_CONFIG['use_images']['obstacles']:
            ...
            if obstacle_img:
                ...
                # Fallback to color
                for obs in env.obstacles:
                    pygame.draw.circle(win, RENDER_CONFIG['colors']['obstacles']['fill'], (int(obs[0]), int(obs[1])), obs[2])
                    pygame.draw.circle(win, RENDER_CONFIG['colors']['obstacles']['border'], (int(obs[0]), int(obs[1])), obs[2], 2)
        else:
            for obs in env.obstacles:
                pygame.draw.circle(win, RENDER_CONFIG['colors']['obstacles']['fill'], (int(obs[0]), int(obs[1])), obs[2])
                pygame.draw.circle(win, RENDER_CONFIG['colors']['obstacles']['border'], (int(obs[0]), int(obs[1])), obs[2], 2)
    ...
    if RENDER_CONFIG['use_images']['robot']:
        ...
        if robot_img:
            ...
            # Fallback to color
            pygame.draw.circle(win, RENDER_CONFIG['colors']['robot']['body'], (int(env.robot_pos[0]), int(env.robot_pos[1])), env.robot_radius)
            ...
            pygame.draw.line(win, RENDER_CONFIG['colors']['robot']['direction'], (int(env.robot_pos[0]), int(env.robot_pos[1])), (int(end_x), int(end_y)), 3)
    else:
        pygame.draw.circle(win, RENDER_CONFIG['colors']['robot']['body'], (int(env.robot_pos[0]), int(env.robot_pos[1])), env.robot_radius)
        end_x = env.robot_pos[0] + (env.robot_radius + 5) * np.cos(np.radians(env.angle))
        end_y = env.robot_pos[1] + (env.robot_radius + 5) * np.sin(np.radians(env.angle))
